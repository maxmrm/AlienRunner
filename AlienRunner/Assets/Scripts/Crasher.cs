...
public class Crasher : MonoBehaviour 
{
	...
	{  
		...
		if (ButtonTypes == "Senstive" )//&& Sb.IsActive)
			ActiveCrasher ();
	...
	void ActiveCrasher()
	{
		...
		if (line == 'X') 
		{
			...
			transform.position += Vector3.right * (MoveSpeed / 60);
		...
		if (line == 'Y')
		{
			transform.position -= Vector3.up * (MoveSpeed / 60);
		...
		if (line == 'Z')
		{
			transform.position += Vector3.forward * (MoveSpeed / 60);
